...
export class PhotoPhotoUpdateComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.albumService
      .query()
      .subscribe((res: HttpResponse<IAlbumPhoto[]>) => (this.albums = res.body), (res: HttpErrorResponse) => this.onError(res.message));
  }
  ...
  trackAlbumById(index: number, item: IAlbumPhoto) {
    return item.id;
  }
}
